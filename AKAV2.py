...
def perform_iterative_search():
    ...
    if products:
        ...
        result_iterative_text.set(f"Iterative: '{target_product}' search completed 10 times.")
        time_iterative_text.set(f"Average Iterative search time: {sum(iterative_times_local) / 10:.6f} seconds")
...
def perform_recursive_search():
    ...
    if products:
        ...
        result_recursive_text.set(f"Recursive: '{target_product}' search completed 10 times.")
        time_recursive_text.set(f"Average Recursive search time: {sum(recursive_times_local) / 10:.6f} seconds")
